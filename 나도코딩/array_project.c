...
int main(void)
{
	...
	for (int i = 1; i <= 3; i++)
	{
		...
		for (int j = 0; j < cntShowBottle; j++)
		{
			...
			{
				j--; //이미 j는 ++이 되어서 j를 한번 壺峠 이렇게해야지 안그러면 3번 시도햇는데 이미 선택된 병이면 새로운 병 선택못하고 끝나기때문.
			}
		}
		...
		{
			if (bottle[k] == 1)
				printf("%d " , k + 1); //k +1 하는 이유 k는 0부터시작해서 보틀이라는 변수는 0번 부터시작해서 0번 2번 병을 사용한다보다는 1번 변수부터 시작하는게 좋아서
		}
		printf(" 물약을 머리에 바릅니다\n\n");
