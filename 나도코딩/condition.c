...
int main(void)
{
	...
	/*for (int i = 1; i <= 30; i++)
	{
		if(i >=6 )
		{
			...
			break; //이 조건이 맞을 for문을 탈출하도록 하는것이 beeak
		}
	...
	if (i >= 6 && i <= 10)
	{
		...
		{
			printf("%d번 학생은 결석입니다\n", i);
			continue; // continue는 이 조건에 들어왔을 그 이후에 있는 문장을 실행하지않고 바로 다음방법 으로넘어감 여기서는 7번 결석출력하고 8로 조별발표 실행함.
		}
		printf("%d 번 학생은 조별 발표 준비를 하세요\n", i);
	}*/
	...
	switch (age)
	{
		...
		case 17:
		case 18:
		case 19:printf("고등학생입니다\n"); break;  //case 어떤 경우를 만낫을 그문장 끝에 break 있으면 이후 case는 실행안하고 끝나지만 break없으면 밑에 케이스는 비교안하고 실행안하기문에 저렇게 압축가능.
