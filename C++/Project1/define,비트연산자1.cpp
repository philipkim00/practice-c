...
10진수 자리수 한칸 올라가는건 10배 커진것.
2진수 자리수 한칸(비트한칸)올라가는건 2배 커진것.

byte <<= 1;
//같은 의미.
byte = byte << 1;
왼쪽으로 한칸 밀고 바이트에 대입한것.
...
13 / 2 = 6.5인데 몫의 값 6
나머지가 없어지는 이유는
0, 0, 0, 0, 1, 1, 0, 1 이 오른쪽으로 밀리면
0, 0, 0, 0, 0, 1, 1, 0 마지막 1이 밀려서 소실됨.
그래서 같은 의미로 나머지를 버리는것.
...
전처리기 과정
#이라고 작성한 전처리기 구문들은 컴파일 과정전에 먼저 처리하고 우리의 코드를본다.

매크로	#define
변수를 특정 숫자로 취환해준다.
#define HUNGRY 1
헝그리라는 코드를 1이라고 바꿔고 컴파일 들어감.

### 전처리기 장점
#### 1.가독성이 높아진다.
변수의 역할을 케릭터가 보유하고있는 여러가지 상태(배고픔, 힘듬, 공격력버프등등)가 있고 중첩되서 돌아간다.
근데 이 많은 상태를 전처리기를 안쓰면 배고픔이 1이고 힘듬이 2고 공격력버프가 27이라는 숫자를 다외워야된다.
이건 말이 안되기 때문이다.

#### 2. 유지보수가 싶다
(헝그리 값을 일일이 다 돌아다니면서 바꾸는게 아니라 매크로 환경만 바꿔주면됨.)
*/